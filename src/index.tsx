...
type State = {
  hoverKey: string,
  width: number,
  xHoverIndex: number | null,
}

export default class StackedAreaChart extends Component<Props,State> {
  static propTypes = {
  ...
  resize = () => {
    if(this.ref.current) {
      this.setState({
        width: this.ref.current.clientWidth, //responsive chart width
      });
    }
  }

  mouseMove = (e: React.MouseEvent<SVGPathElement | SVGRectElement>, key: string) => {
    if(this.ref.current) {
      const boundings = this.ref.current.getBoundingClientRect(); //get the bounds of the chart
      const xHoverIndex = this.mapPositionToIndex(e.clientX - boundings.left); //calculate the x hover index
      if(xHoverIndex!==this.state.xHoverIndex || key!==this.state.hoverKey) { //if we should set a new xHoverIndex or hoverKey
        this.setState({hoverKey: key, xHoverIndex: xHoverIndex});

        this.props.onMouseOverHandler(e, key, xHoverIndex); //run the callback
      }
    }
  }

  mapPositionToIndex = (x: number) => {
    const {
      data,
      margin,
    } = this.props

    const domain = this.state.width - margin.left - margin.right;
  ...
  mouseLeave = () => {
    this.setState({xHoverIndex: null});

    this.props.onMouseOverHandler(null);
  }

  getTitle = (key:string, dateFormat: (date:Date) => string) => {
    const data = this.props.data
    const xHoverIndex = this.state.xHoverIndex
  ...
  render() {
    ...
    const yDomain:[number, number] = [0, max(series, d => max(d, d => d[1])) || 0];
    ...
    const rectWidth = Math.min(500, (width-margin.left-margin.right)/2)

    return (
      <div ref={this.ref}>
        <svg width={width} height={height} onMouseLeave={this.mouseLeave}>
          <g>
            {data.map((d,i) => {
              if(i%xTicksSkip === 0) {
                return (
          ...
          <g>
            {yAxis.map((yValue,i) =>
              <g key={i}>
                <line key={i} x1={margin.left-yTickSize} x2={this.state.width-margin.right} y1={y(yValue)} y2={y(yValue)} strokeWidth={axisStrokeWidth} stroke={axisStroke}></line>
              ...
              if(d[0]) {
                ...
                return (
                  <Element
                    ...
                    opacity={areaOpacity}
                    onClick={(e: React.MouseEvent<SVGPathElement | SVGRectElement>)  => this.props.onClickHandler(e, d.key, xHoverIndex)}
                    ...
                    onMouseMove={(e: React.MouseEvent<SVGPathElement | SVGRectElement>) => this.mouseMove(e, d.key)}
                  >
                    <title>{this.getTitle(d.key, dateFormat)}</title>
                  </Element>
                )
              }
              return null
            ...
            <g>
              ...
              {keys.map((k,i) =>
                <circle
                  ...
                  filter={dotsFilter}
                >
                  <title>{this.getTitle(k, dateFormat)}</title>
                </circle>
              )}
            </g>
          : null}
