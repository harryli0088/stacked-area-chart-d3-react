...
test("one data point, one key-value", () => {
  ...
  const result = getYAxisData(data, ["apple"], 1000, NO_MARGIN, 5)
...
test("one data point, multiple key-values", () => {
  ...
  const result = getYAxisData(data, ["apple","banana","cherries"], 1000, NO_MARGIN, 10)
  expect(result.yTicks).toEqual([
...
test("ignore invalid keys", () => {
  const data:DataPointType[] = [
    {date: new Date(), apple: 20, banana: 60, cherries: 50},
    {date: new Date(), apple: 70, banana: 50, cherries: 70}, //max sum 120
    {date: new Date(), apple: 10, banana: 90, cherries: 20},
  ]

  //cherries is not included and should be ignored
  const result = getYAxisData(data, ["apple","banana"], 1000, NO_MARGIN, 10)
  expect(result.yTicks).toEqual([
    0, 20, 40, 60, 80, 100, 120
  ])
  expect(result.yScale(0)).toEqual(1000)
  expect(result.yScale(30)).toEqual(750)
  expect(result.yScale(60)).toEqual(500)
  expect(result.yScale(90)).toEqual(250)
  expect(result.yScale(120)).toEqual(0)
})
